...
###############################################################################
# cint — Cyber Intelligence — pi image
#
# Stages:
...
#                     + /usr/local/bin/cint shim
#   pi-runtime      — pi-base + pi source + bun install      (DEFAULT, runnable)
#
# Build:
#     docker build -t incrt/cint:dev .                          # default = pi-runtime
#     docker build --target pi-base -t incrt/cint-base:dev .    # base for derived images
#
#
# Run:
...
RUN printf '%s\n' \
    ...
    > /usr/local/bin/cint \
    && chmod +x /usr/local/bin/cint

############################
# 4) pi-runtime — pi-base + pi source + bun install (DEFAULT)
#
# A self-contained, runnable cint image. `docker run incrt/cint:dev --help`
...
ENTRYPOINT ["/usr/bin/tini", "--", "/usr/local/bin/cint"]
CMD ["--help"]
