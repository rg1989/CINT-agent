...
// (native addon, mupdf).
const ALWAYS_EXTERNAL = ["mupdf", "@incrt/cint-natives", "@huggingface/transformers", "fastembed", "onnxruntime-node"];
...
async function cleanBundleOutputs(): Promise<void> {
	// dist/ is shared with the dev binary (dist/cint); only remove this
