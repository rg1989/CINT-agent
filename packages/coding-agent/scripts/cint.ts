/**
 * Bun `--preload` shim for the cint dev launcher (`scripts/cint`).
 *
 * The launcher starts Bun from an empty, bunfig-free directory so a foreign
 * project's `bunfig.toml` `preload` cannot run inside the cint CLI: Bun reads
 ...
 * import-time snapshots (e.g. `getProjectDir()` in `@incrt/cint-utils/dirs`).
 */
const launchCwd = process.env.CINT_LAUNCH_CWD;
if (launchCwd) {
	delete process.env.CINT_LAUNCH_CWD;
	try {
