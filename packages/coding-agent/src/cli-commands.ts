 ...
 */
import type { CommandEntry } from "@incrt/cint-utils/cli";
...
// commands. Without a guard `resolveCliArgv` rewrites e.g. `cint list` to
// `cint launch list`, silently forwarding the bare verb to the model as a prompt
...
const RESERVED_TOP_LEVEL_WORDS = new Map<string, string>([
	[
		"extensions",
		'`cint extensions` is not a management command. Use `cint plugin list` / `cint plugin install`, or run `cint launch extensions` if you meant to send "extensions" as a prompt.',
	],
	[
		"list",
		'`cint list` is not a top-level command. Use `cint plugin list` to list installed plugins, or run `cint launch list` if you meant to send "list" as a prompt.',
	],
	[
		"remove",
		'`cint remove` is not a top-level command. Use `cint plugin uninstall <name>` to remove a plugin, or run `cint launch remove` if you meant to send "remove" as a prompt.',
	],
]);
