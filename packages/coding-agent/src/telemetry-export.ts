 ...
 *
 * oh-my-pi's agent core (`@incrt/cint-agent-core`) emits OpenTelemetry GenAI
 ...
 */
import { logger, postmortem } from "@incrt/cint-utils";
...
async function registerProvider(): Promise<void> {
	...
	const tracerProvider = new NodeTracerProvider({
		resource: resourceFromAttributes({
			"service.name": process.env.OTEL_SERVICE_NAME ?? "cint",
		}),
