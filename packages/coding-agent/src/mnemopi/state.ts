...
import type * as MnemopiNs from "@incrt/cint-mnemopi";
import type { Mnemopi, RecallResult } from "@incrt/cint-mnemopi";
import type * as MnemopiCoreNs from "@incrt/cint-mnemopi/core";
import type { LocalModelInitializer } from "@incrt/cint-mnemopi/core";
...
/**
 * Lazily load `@incrt/cint-mnemopi` (memoized) and route fastembed loads
...
export async function loadMnemopi(): Promise<typeof MnemopiNs> {
	if (!mnemopiMod) {
		mnemopiMod = await import("@incrt/cint-mnemopi");
		installLocalModelInitializer(mnemopiMod.setLocalModelInitializer);
	}
	return mnemopiMod;
}

/** Lazily load `@incrt/cint-mnemopi/core` (memoized). */
export async function loadMnemopiCore(): Promise<typeof MnemopiCoreNs> {
	if (!mnemopiCoreMod) {
		mnemopiCoreMod = await import("@incrt/cint-mnemopi/core");
