...
import { normalizeProfileName } from "@incrt/cint-utils/dirs";
...
function validateAliasName(aliasName: string, shell: ProfileAliasShell): string {
	...
	if (normalized.toLowerCase() === "cint") {
		throw new Error('Invalid alias "cint". Refusing to shadow the base cint command.');
	}
