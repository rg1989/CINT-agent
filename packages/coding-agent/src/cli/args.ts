 ...
 */
import { APP_NAME, CONFIG_DIR_NAME, logger } from "@incrt/cint-utils";
 ...
 * setter table itself can stay free of `@incrt/cint-utils` runtime imports
...
export function getExtraHelpText(): string {
  ...
  OMP_PROFILE / CINT_PROFILE   - Named profile for isolated agent state (same as --profile)
  Use \`cint --profile <name> --alias <command>\` to create a shell shortcut for a profile
...
${chalk.bold("Useful Commands:")}
  cint agents unpack           - Export bundled subagents to ~/.cint/agent/agents (default)
  cint agents unpack --project - Export bundled subagents to ./.cint/agents`;
}
