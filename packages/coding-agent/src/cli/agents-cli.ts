 ...
 *
 * Handles `cint agents unpack` for writing bundled agent definitions to disk.
 */
import * as fs from "node:fs/promises";
import * as path from "node:path";
import { getAgentDir, getProjectDir, isEnoent } from "@incrt/cint-utils";
...
function resolveTargetDir(flags: AgentsCommandArgs["flags"]): string {
	...
	if (flags.project) {
		return path.resolve(getProjectDir(), ".cint", "agents");
	}
