...
import { $which, APP_NAME, isEnoent, VERSION } from "@incrt/cint-utils";
import { $ } from "bun";
 ...
 */
const NATIVES_PACKAGE = "@incrt/cint-natives";

/**
 * Platform tags the release pipeline publishes as
 * `@incrt/cint-natives-<tag>` leaves. Mirrors `SUPPORTED_PLATFORMS` in
 ...
 * `@incrt/cint-natives-<tag>` leaf to `expectedVersion`. `bun install -g`
