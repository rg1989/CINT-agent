...
import { truncateToWidth } from "@incrt/cint-tui/utils";
import { APP_NAME, formatDuration, formatNumber, formatPercent } from "@incrt/cint-utils";
...
async function printStatsSummary(): Promise<void> {
	const { getDashboardStats } = await import("@incrt/cint-stats");
	const stats = await getDashboardStats();
