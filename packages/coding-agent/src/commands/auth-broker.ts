/**
 * `cint auth-broker` — manage the cint credential vault.
 */
...
export default class AuthBroker extends Command {
	static description = "Manage the cint auth-broker (credential vault)";
	...
	static examples = [
		"# Boot the broker against the local SQLite store\n  cint auth-broker serve",
		"# Boot on a non-default port\n  cint auth-broker serve --bind=127.0.0.1:9000",
		"# Print the bearer token\n  cint auth-broker token",
		"# Rotate the bearer token\n  cint auth-broker token --regenerate",
		"# List supported OAuth providers\n  cint auth-broker list",
		"# Local login (run on the broker host)\n  cint auth-broker login anthropic",
		"# Interactive provider selection\n  cint auth-broker login",
		"# Remote login over SSH tunnel\n  cint auth-broker login anthropic --via=user@broker",
		"# Log out of a provider (interactive without provider arg)\n  cint auth-broker logout anthropic",
		"# Import a CLIProxyAPI auth dump\n  cint auth-broker import ~/.cliproxy/auth",
		"# Import a single CLIProxyAPI JSON, overriding the provider mapping\n  cint auth-broker import ~/.cliproxy/auth/claude-foo.json --provider anthropic",
		"# Preview a migration from local store + env vars to the configured broker\n  cint auth-broker migrate --from-local --include-env --dry-run",
		"# Apply the migration\n  cint auth-broker migrate --from-local --include-env",
		"# Health-check the configured remote broker\n  cint auth-broker status",
	];

	async run(): Promise<void> {
		const { args, flags } = await this.parse(AuthBroker);
		if (!args.action) {
			renderCommandHelp("cint", "auth-broker", AuthBroker);
			return;
		}
