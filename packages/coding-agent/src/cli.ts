...
import { declareWorkerHostEntry, installWorkerInbox } from "@incrt/cint-utils/worker-host";
...
// `@incrt/cint-utils/env` eagerly loads `.env` from the agent directory at
// import time, so it must not be imported before `setProfile` runs.

async function showHelp(config: CliConfig): Promise<void> {
	const { renderRootHelp } = await import("@incrt/cint-utils/cli");
...
async function runSmokeTest(): Promise<void> {
	const { smokeTestSyncWorker, startServer } = await import("@incrt/cint-stats");
...
async function runWorkerEntrypoint(arg: string | undefined): Promise<boolean> {
	...
	if (arg === STATS_SYNC_WORKER_ARG) {
		...
		await import("@incrt/cint-stats/sync-worker");
...
export async function runCli(argv: string[]): Promise<void> {
	...
	// `@incrt/cint-utils/env` here would snapshot the wrong agent `.env`.
