...
import { TtsrManager } from "@incrt/cint/export/ttsr";
...
import { Agent, AgentBusyError, type AgentMessage, type AgentTool } from "@incrt/cint-agent-core";
import type { AssistantMessage, Message, ToolCall } from "@incrt/cint-ai";
...
describe("AgentSession TTSR resume gate", () => {
	...
	it("relativizes the rule file path in the TTSR interrupt injection (no absolute leak)", async () => {
		...
		const ruleAbsPath = path.join(cwd, ".cint", "rules", "no-unwrap.md");
