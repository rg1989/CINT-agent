...
// exposes the same surface at the real `@incrt/cint-ai/oauth` export, so the
// legacy `@mariozechner/pi-ai/oauth` specifier canonicalizes straight to it.
const canonicalAiOauth = Bun.resolveSync("@incrt/cint-ai/oauth", import.meta.dir);
...
const CASES: readonly AliasCase[] = [
	...
	// Subpath: legacy `pi-ai/oauth` resolves to the real `@incrt/cint-ai/oauth`.
	{
