...
import type { SessionManager } from "@incrt/cint-coding-agent/session/session-manager";
import { executeAcpBuiltinSlashCommand } from "@incrt/cint-coding-agent/slash-commands/acp-builtins";
...
describe("wave 5 — adapters and polish", () => {
	...
	it("/mcp add foo --url https://example.com --token X --scope project: outputs success or propagates write error", async () => {
		...
		const mcpModule = await import("@incrt/cint-coding-agent/mcp/config-writer");
	...
	it("/ssh add foo --host x --user y --scope user: calls addSSHHost", async () => {
		const sshModule = await import("@incrt/cint-coding-agent/ssh/config-writer");
	...
	it("/marketplace discover: output is bulleted with '  - ' token", async () => {
		const { MarketplaceManager } = await import("@incrt/cint-coding-agent/extensibility/plugins/marketplace");
