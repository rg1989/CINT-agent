 ...
 *
 * Note: helpers.ts imports @incrt/cint-natives (Rust addon via glob).
...
describe("resolveActiveProjectRegistryPath", () => {
	...
	it("walk-up finds nearest .omp/ directory", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".cint"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, ".cint", "plugins", "installed_plugins.json"));
	});
	...
	it("walk-up stops at the nearest .omp/ — does not skip to a more distant one", async () => {
		...
		fs.mkdirSync(path.join(tmpDir, ".cint"), { recursive: true });
		fs.mkdirSync(path.join(tmpDir, "sub", ".cint"), { recursive: true });
		const cwd = path.join(tmpDir, "sub", "nested");
		fs.mkdirSync(cwd, { recursive: true });

		const result = await resolveActiveProjectRegistryPath(cwd);

		expect(result).toBe(path.join(tmpDir, "sub", ".cint", "plugins", "installed_plugins.json"));
	});
	...
	it("does not treat ~/.git as a project root (pass-2 home-dir guard)", async () => {
		...
		try {
			...
			const homeOmpPath = path.join(homeDir, ".cint", "plugins", "installed_plugins.json");
...
describe("listClaudePluginRoots — project shadows user", () => {
	...
	beforeEach(() => {
		...
		fs.mkdirSync(path.join(tmpProject, ".cint", "plugins"), { recursive: true });

		userRegPath = path.join(tmpHome, ".cint", "plugins", "installed_plugins.json");
		fs.mkdirSync(path.dirname(userRegPath), { recursive: true });

		projectRegPath = path.join(tmpProject, ".cint", "plugins", "installed_plugins.json");
	});
