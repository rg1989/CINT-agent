...
import { createTinyTitleSubprocess, TINY_WORKER_ARG } from "@incrt/cint-coding-agent/tiny/title-client";
...
describe("issue #1606 — tiny model lives in an isolated subprocess", () => {
	it("ping/pongs through the spawned worker subprocess and tears it down cleanly", async () => {
		...
		const script =
			'const { smokeTestTinyTitleWorker } = await import("@incrt/cint-coding-agent/tiny/title-client"); await smokeTestTinyTitleWorker({ timeoutMs: 15000 });';
