...
describe("SYSTEM.md prompt assembly", () => {
	...
	it("renders SYSTEM.md exactly once when it is used as the custom base prompt", async () => {
		...
		const systemDir = path.join(projectDir, ".cint");
	...
	it("suppresses discovered SYSTEM.md while preserving the project footer", async () => {
		...
		fs.mkdirSync(path.join(projectDir, ".cint"), { recursive: true });
		fs.writeFileSync(path.join(projectDir, ".cint", "SYSTEM.md"), "Discovered project SYSTEM prompt");
	...
	it("prefers project SYSTEM.md over user SYSTEM.md", async () => {
		const projectDir = path.join(tempDir, "project");
		fs.mkdirSync(path.join(projectDir, ".cint"), { recursive: true });
		fs.mkdirSync(path.join(tempHomeDir, ".cint", "agent"), { recursive: true });
		fs.writeFileSync(path.join(tempHomeDir, ".cint", "agent", "SYSTEM.md"), "User SYSTEM prompt");
		fs.writeFileSync(path.join(projectDir, ".cint", "SYSTEM.md"), "Project SYSTEM prompt");
