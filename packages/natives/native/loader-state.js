...
/**
 * Native addon loader for `@incrt/cint-natives`.
 *
...
function resolveLeafPackageDir(platformTag) {
	try {
		...
		return path.dirname(require_.resolve(`@incrt/cint-natives-${platformTag}/package.json`));
	} catch {
		return null;
	}
}
 ...
 * `node_modules/@incrt/cint-natives/native/`, leaving an old binary next to a
...
function validateLoadedBindings(ctx, bindings, candidate) {
	...
	throw new Error(
		`Loaded ${candidate} but it does not expose the @incrt/cint-natives@${ctx.packageVersion} ` +
...
function buildHelpMessage(ctx) {
	...
	return (
		"If installed via npm/bun, try reinstalling: bun install @incrt/cint-natives\n" +
