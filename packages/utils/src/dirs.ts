/**
 * Centralized path helpers for cint config directories.
 *
 * Uses PI_CONFIG_DIR (default ".cint") for the config root and
 * PI_CODING_AGENT_DIR to override the agent directory.
 *
 * On Linux, if XDG_DATA_HOME / XDG_STATE_HOME / XDG_CACHE_HOME environment
 * variables are set, paths are redirected to XDG-compliant locations under
 * $XDG_*_HOME/cint/. This requires running `cint config migrate` first to
 * move data to the new locations. No filesystem existence checks are performed
 * — if the env var is set, cint trusts that the migration has been done.
 */
...
/** App name (e.g. "cint") */
export const APP_NAME: string = "cint";

/** Config directory name (e.g. ".cint") */
export const CONFIG_DIR_NAME: string = ".cint";
...
const PROFILE_ENV_KEYS = ["CINT_PROFILE", "OMP_PROFILE", "PI_PROFILE"] as const;
 ...
 *
 * Exported so consumers of `@incrt/cint-utils/dirs` (CLI bootstrap, tests,
