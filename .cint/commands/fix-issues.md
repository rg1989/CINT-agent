...
2. `gh search prs` for the issue number to see if a fix is already in flight.
   - If a PR exists and looks reasonable → switch tracks: review that PR per `.cint/commands/review-prs.md` instead, and report back as `existing-pr`. Do **not** open a competing fix.
...
Branch naming: `fix/issue-<N>` (or `fix/issue-<N>-<slug>` if you'll open multiple). Path under `~/.cint/wt/<encoded-main-path>/...` matches the convention `pr_checkout` uses.
...
Worktree:  ~/.cint/wt/.../fix-issue-<N>            (if created)
