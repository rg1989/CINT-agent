...
`scripts/probe.ts` routes through `@incrt/cint-ai` (`completeSimple`) so model/auth/provider behavior matches production.

```bash
bun .cint/skills/tool-prompt-optimization/scripts/probe.ts \
...
```bash
bun .cint/skills/tool-prompt-optimization/scripts/probe-builtin.ts --tool <name> [--no-summary] [--show]
```
...
```ts
import { arkToWireSchema } from "@incrt/cint-ai"; // or toolWireSchema(tool)
